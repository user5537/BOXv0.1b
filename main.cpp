...
case 8:
//wyswietla wszystkie wiadomosci
cout<<setw(15)<<"SMS+"<<endl;

//cout<< strlen(w[9])<<endl;
*w[9].tresc="NOWO wygeneowany sms";
...
cin >> y;
    ...
    if (y==1)
     { for(int p=1;p<sizeof(w)/sizeof(w[0]);p++)
            *(w[p-1].tresc)=*(w[p].tresc);
*(w[9].tresc)="";
break;
}

     if  (y==2)
    {  for(int p=2;p<sizeof(w)/sizeof(w[0]);p++)
            *(w[p-1].tresc)=*(w[p].tresc);
*(w[9].tresc)="";
break;
}
    if  (y==3)
    {  for(int p=3;p<sizeof(w)/sizeof(w[0]);p++)
            *(w[p-1].tresc)=*(w[p].tresc);
*(w[9].tresc)="";
break;
}
    ...
    if  (y==4)
    {  for(int p=4;p<sizeof(w)/sizeof(w[0]);p++)
            *(w[p-1].tresc)=*(w[p].tresc);
*(w[9].tresc)="";
break;
}
    if  (y==5)
    {  for(int p=5;p<sizeof(w)/sizeof(w[0]);p++)
            *(w[p-1].tresc)=*(w[p].tresc);
*(w[9].tresc)="";
break;
}
    if  (y==6)
    {  for(int p=6;p<sizeof(w)/sizeof(w[0]);p++)
            *(w[p-1].tresc)=*(w[p].tresc);
*(w[9].tresc)="";
break;
}
    if  (y==7)
    {  for(int p=7;p<sizeof(w)/sizeof(w[0]);p++)
            *(w[p-1].tresc)=*(w[p].tresc);
*(w[9].tresc)="";
break;
}
    if  (y==8)
    {  for(int p=8;p<sizeof(w)/sizeof(w[0]);p++)
            *(w[p-1].tresc)=*(w[p].tresc);
    *(w[9].tresc)="";
...
//for(int j=p;j<sizeof(w)/sizeof(w[0])-1;j++)
